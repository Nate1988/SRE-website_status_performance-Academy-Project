...
def dashboard():
    with tracer.start_as_current_span("dashboard_view"):
        ...
        return render_template_string(HTML_TEMPLATE, results=safe_results)

@app.route('/metrics')
def metrics():
    # Return the latest metrics to Prometheus
    return Response(generate_latest(), mimetype=CONTENT_TYPE_LATEST)

# --- Start Application ---
if __name__ == '__main__':
    # Run one check initially before starting background thread
    check_websites_once = threading.Thread(target=check_websites, daemon=True)
    # Perform initial check synchronously
    def initial_check():
        logging.info("Performing initial website check...")
        current_results = []
    ...
    initial_check()
    # Start the periodic checker thread
    checker_thread = threading.Thread(target=check_websites, daemon=True)
